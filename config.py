...
class Config(BaseModel):
    chat_model: str = Field(
        default=os.getenv("CHAT_MODEL"), description="Chat model", frozen=True
    )
    ...
    chat_api_key: str = Field(
        default=os.getenv("OPENAI_API_KEY"), description="Chat API key", frozen=True
    )
    embedding_model: str = Field(
        default=os.getenv("EMBEDDING_MODEL"), description="Embedding model", frozen=True
    )
    pdf_dir: str = Field(
